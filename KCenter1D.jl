...
function KCenter1D(y::Array{Float64,1},K::Int64):
	...
	if (K>length(unique(y)) || length(set(y))==1)
		return (ASCIIString=>Float64)["Cluster"=>fill(1,length(y)),"Centers"=>y,"Size"=>fill(1,K)]
	end
	...
	for k=1:K
		...
		for i=2:N
			...
			#when k > 1, D(k,i)  = min ( D(k-1,j-1) + d(j,i) ) for j>=1
				...
				D(k,i) = None  			#value of D(k,i) is not set
	...
	cluster_right = N
	cluster_left  = -1
	Cluster 			= Array{Int64}(N)   # record which cluster each point belongs to
	...
	for k=K:-1:1
		...
		for i=cluster_left:cluster_right+1
			Cluster[i] = k
		end
		...
		for a=cluster_left:cluster_right+1
			WithinSS[k] += (y[a] - Center[k])^2
		end

		Size[k] = cluster_right - Int64(cluster_left) + 1
	...
	return (ASCIIString=>Float64)["Cluster"=>Clusters,"Centers"=>Center,"Size"=>Size,"WithinSS"=>WithinSS]
end